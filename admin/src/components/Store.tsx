...
import AdminOrders from './AdminOrders'       // orders table
...
const tabs = [
  ...
  { key: 'orders',      label: 'Orders', icon: ReceiptText, grad: 'linear-gradient(90deg,#FA9F2C,#FCD62B)' },
  { key: 'return', label: 'Returns', icon: Boxes, grad: 'linear-gradient(90deg,#F53C44,#FA7236)' },
] as const
...
const Store: React.FC = () => {
  ...
  const active = useMemo(() => {
    ...
    return t === 'add' || t === 'orders' || t == 'collections'? t:'return'
  }, [params])

  const setTab = (key: 'collections' | 'add' | 'orders'|'return') => {
